...
func TestMachineToInfrastructureMapFunc(t *testing.T) {
	...
	var testcases = []struct {
	...
	}{
		{
			...
			input: schema.GroupVersionKind{
				Group:   "foo.cluster.getupcloud.com",
				Version: "v1alpha1",
				Kind:    "TestMachine",
			},
			request: handler.MapObject{
				Object: &clusterv1.Machine{
					...
					Spec: clusterv1.MachineSpec{
						InfrastructureRef: corev1.ObjectReference{
							APIVersion: "foo.cluster.getupcloud.com/v1alpha1",
							Kind:       "TestMachine",
							Name:       "infra-1",
						},
					},
		...
		},
		{
			...
			request: handler.MapObject{
				Object: &clusterv1.Machine{
					...
					Spec: clusterv1.MachineSpec{
						InfrastructureRef: corev1.ObjectReference{
							APIVersion: "bar.cluster.getupcloud.com/v1alpha1",
							Kind:       "TestMachine",
							Name:       "bar-1",
						},
					},
...
func TestGetOwnerClusterSuccessByName(t *testing.T) {
	...
	objm.OwnerReferences[0].APIVersion = "cluster.getupcloud.com/v1alpha1234"
	cluster, err = GetOwnerCluster(context.TODO(), c, objm)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(cluster).NotTo(BeNil())
}
